...
class ProductsController {
 ...
 featured = async (req,res,next) => {
  ...
  latest = async (req,res,next) => {
    ...
    try{
        const products = await Product.find({status:true}).sort({createAt:'desc'})
  ...
  topSelling = async (req,res,next) => {
    ...
    try{
        ...
        .match({status:true})
        .lookup({from:'details', localField:'_id', foreignField:'productId', as:'detail_count'})
